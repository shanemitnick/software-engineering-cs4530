...
export function conversationAreaCreateHandler(
  ...
  // retrieve token and town controlled
  ...
  // check if token is valid and equal to session token
  if (s?.sessionToken === token) {
    const conversionCreationSuccessful = townController?.addConversationArea(
      _requestData.conversationArea,
    );

    // check if conversion area was successfully created.
    // if not, code will continue to default.
    if (conversionCreationSuccessful === true) {
      return {
        isOK: true,
        response: {},
        message: undefined,
  ...
  const message = `Unable to create conversation area ${_requestData.conversationArea.label} with topic ${_requestData.conversationArea.topic}`;
  return {
    isOK: false,
    response: {},
    message,
