...
export function conversationAreaCreateHandler(_requestData: ConversationAreaCreateRequest): ResponseEnvelope<Record<string, null>> {
  if (stryMutAct_9fa48("89")) {
    ...
    if (stryMutAct_9fa48("92") ? false : stryMutAct_9fa48("91") ? true : stryMutAct_9fa48("90") ? townController?.getSessionByToken(_requestData.sessionToken) : (stryCov_9fa48("90", "91", "92"), !(stryMutAct_9fa48("93") ? townController.getSessionByToken(_requestData.sessionToken) : (stryCov_9fa48("93"), townController?.getSessionByToken(_requestData.sessionToken))))) {
      if (stryMutAct_9fa48("94")) {
        {}
      } else {
        stryCov_9fa48("94");
        return stryMutAct_9fa48("95") ? {} : (stryCov_9fa48("95"), {
          isOK: stryMutAct_9fa48("96") ? true : (stryCov_9fa48("96"), false),
          response: {},
          message: stryMutAct_9fa48("97") ? `` : (stryCov_9fa48("97"), `Unable to create conversation area ${_requestData.conversationArea.label} with topic ${_requestData.conversationArea.topic}`)
        });
      }
    }
    ...
    return stryMutAct_9fa48("98") ? {} : (stryCov_9fa48("98"), {
      isOK: success,
      response: {},
      message: (stryMutAct_9fa48("99") ? success : (stryCov_9fa48("99"), !success)) ? stryMutAct_9fa48("100") ? `` : (stryCov_9fa48("100"), `Unable to create conversation area ${_requestData.conversationArea.label} with topic ${_requestData.conversationArea.topic}`) : undefined
    });
  }
}
/**
...
export function townSubscriptionHandler(socket: Socket): void {
  ...
  const townController = CoveyTownsStore.getInstance().getControllerForTown(coveyTownID); // Retrieve our metadata about this player from the TownController
  ...
  townController.addTownListener(listener); // Register an event listener for the client socket: if the client disconnects,
