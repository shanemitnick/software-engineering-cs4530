...
describe('CoveyTownController', () => {
  ...
  describe('addPlayerToConversationArea', () => {
    ...
    expect(player.location.x).toBe(25);
    expect(player.location.y).toBe(25);
  ...
  describe('boxOverlap', () => {
    const ca1 = TestUtils.createConversationForTesting({
      boundingBox: { x: 10, y: 10, height: 10, width: 10 },
    });

    const ca2 = TestUtils.createConversationForTesting({
      boundingBox: { x: 50, y: 50, height: 50, width: 50 },
    });

    const ca3 = TestUtils.createConversationForTesting({
      boundingBox: { x: 50, y: 50, height: 100, width: 100 },
    });

    const ca4 = TestUtils.createConversationForTesting({
      boundingBox: { x: 30, y: 10, height: 10, width: 30 },
    });

    const ca5 = TestUtils.createConversationForTesting({
      boundingBox: { x: 35, y: 20, height: 10, width: 10 },
    });

    const ca6 = TestUtils.createConversationForTesting({
      boundingBox: { x: 55, y: 15, height: 20, width: 30 },
    });

    test('testing boxOverlay', () => {
      // expect(testingTown.boxOverlap(ca1.boundingBox, ca1.boundingBox)).toBeTruthy();
      expect(CoveyTownController.boxOverlap(ca1.boundingBox, ca2.boundingBox)).toBeFalsy();
      expect(CoveyTownController.boxOverlap(ca2.boundingBox, ca1.boundingBox)).toBeFalsy();

      expect(CoveyTownController.boxOverlap(ca3.boundingBox, ca1.boundingBox)).toBeTruthy();
      expect(CoveyTownController.boxOverlap(ca1.boundingBox, ca3.boundingBox)).toBeTruthy();

      // // sharing border
      expect(CoveyTownController.boxOverlap(ca1.boundingBox, ca4.boundingBox)).toBeFalsy();
      expect(CoveyTownController.boxOverlap(ca4.boundingBox, ca1.boundingBox)).toBeFalsy();

      expect(CoveyTownController.boxOverlap(ca6.boundingBox, ca1.boundingBox)).toBeFalsy();
      expect(CoveyTownController.boxOverlap(ca6.boundingBox, ca2.boundingBox)).toBeFalsy();
      expect(CoveyTownController.boxOverlap(ca6.boundingBox, ca5.boundingBox)).toBeTruthy();
      expect(CoveyTownController.boxOverlap(ca6.boundingBox, ca4.boundingBox)).toBeTruthy();

      // expect(CoveyTownController.boxOverlap(ca1.boundingBox, ca3.boundingBox)).toBeTruthy();
      // expect(CoveyTownController.boxOverlap(ca2.boundingBox, ca3.boundingBox)).toBeTruthy();
      // expect(CoveyTownController.boxOverlap(ca4.boundingBox, ca3.boundingBox)).toBeTruthy();
      // expect(CoveyTownController.boxOverlap(ca5.boundingBox, ca3.boundingBox)).toBeTruthy();
      // expect(CoveyTownController.boxOverlap(ca6.boundingBox, ca3.boundingBox)).toBeTruthy();

      // expect(CoveyTownController.boxOverlap(ca3.boundingBox, ca1.boundingBox)).toBeTruthy();
      // expect(CoveyTownController.boxOverlap(ca3.boundingBox, ca2.boundingBox)).toBeTruthy();
      // expect(CoveyTownController.boxOverlap(ca3.boundingBox, ca4.boundingBox)).toBeTruthy();
      // expect(CoveyTownController.boxOverlap(ca3.boundingBox, ca5.boundingBox)).toBeTruthy();
      // expect(CoveyTownController.boxOverlap(ca3.boundingBox, ca6.boundingBox)).toBeTruthy();
    });
  });
